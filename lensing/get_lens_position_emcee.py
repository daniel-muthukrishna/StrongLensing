...
def run_mcmc(x_img, y_img, fig_dir, d=None, ndim=5, nwalkers=100, nsteps=200, prior_func=None, initial_params=None,
             fits_file=None, img_name=''):
...
def macs0451_multiple_sources():
    ...
    # img_xobs['62'] = np.array([3486.371962])
    # img_yobs['62'] = np.array([3069.305065])
    # d['62'] = scale_einstein_radius(z_lens=z_lens, z_src=1.405)
    ...
    def lnprior(pars):
        ...
        if (2400 < xlens < 4000) and (2000 < ylens < 3700) and (-np.pi < theta < np.pi) and (10 < b < 2000) and (
                0. < q < 1):
            return 0.0
        else:
            return -np.inf
    ...
    run_mcmc(img_xobs, img_yobs, fig_dir, d, ndim=ndim, nwalkers=nwalkers, nsteps=nsteps, prior_func=lnprior,
             initial_params=initial, fits_file=fits_file, img_name=img_name)
