...
def deflections(x_img, y_img, d, pars):
    ...
    xout, yout = sie_model(x, y, d, pars)  # Calculated deflection positions in lens plane coordinates
...
def pred_positions(x_img, y_img, d, pars):
    ...
    x_deflect, y_deflect = deflections(x, y, d, pars)  # Calculated deflection positions in image plane coordinates (pixel positions)
...
def lnlike(pars, x_img, y_img, d):
    ...
    if isinstance(x_img, dict):
        ...
        for key in x_img:
            x_src[key], y_src[key] = pred_positions(x_img[key], y_img[key], d[key], pars)
            lnlike_dict[key] = -0.5 * (x_src[key].var() + y_src[key].var())

        return sum(lnlike_dict.values())

    else:
        x_src, y_src = pred_positions(x_img, y_img, d, pars)
...
def lnprob(pars, x_img, y_img, d, prior_func=None):
    """ Log-probability is the log-prior + log-likelihood. """
    lp = lnprior(pars, prior_func)

    if not np.isfinite(lp):
        return -np.inf
    return lp + lnlike(pars, x_img, y_img, d)
